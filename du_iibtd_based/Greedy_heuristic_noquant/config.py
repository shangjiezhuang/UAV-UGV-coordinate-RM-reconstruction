"""
Configuration for Greedy UAV-UGV cooperative spectrum sensing.
...
class UAVConfig:
    ...
    step_size: float = 4.0                        # Movement distance per step (grid cells);
...
class UGVConfig:
    ...
    num_directions: int = 5                        # 4 cardinal directions + stay
    step_size: float = 5.0                        # Movement distance per step (grid cells);
...
class ObservationConfig:
    ...
    num_planner_features: int = 6                 # target_dir(x,y), center_freq, score, dist_uav, dist_ugv
...
class PlannerConfig:
    ...
    hybrid_global_step_multiplier: int = 9        # Stay in global mode for this many ensemble intervals before local re-check
...
@dataclass
class RuntimeConfig:
    """Runtime settings kept under mappo for environment compatibility."""
    num_envs: int = 1
...
class Config:
    ...
    mappo: RuntimeConfig = field(default_factory=RuntimeConfig)
    ...
    def __post_init__(self):
        ...
        _ensure(
            self.obs.num_planner_features > 0,
            "obs.num_planner_features must be positive, got "
        ...
        self.planner.hybrid_global_step_multiplier = int(self.planner.hybrid_global_step_multiplier)
        ...
        _ensure(
            self.planner.hybrid_global_step_multiplier > 0,
            "planner.hybrid_global_step_multiplier must be positive, got "
            f"{self.planner.hybrid_global_step_multiplier}",
        )
        _ensure(

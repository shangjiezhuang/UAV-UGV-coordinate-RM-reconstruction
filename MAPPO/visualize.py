...
def plot_reward_components(
    ...
    reward_keys = [
        ...
        ('r_tx', 'TX Penalty', '#3498db'),
        ('r_queue', 'Queue Penalty', '#e74c3c'),
        ('r_comm', 'Comm Penalty', '#7f8c8d'),
        ('r_terminal', 'Terminal Reward', '#8e44ad'),
    ]

    num_cols = 2
    num_rows = int(np.ceil(len(reward_keys) / num_cols))
    fig, axes = plt.subplots(num_rows, num_cols, figsize=(14, 4.2 * num_rows))
    axes = np.atleast_1d(axes).flatten()

    for idx, (key, label, color) in enumerate(reward_keys):
        ax = axes[idx]
    ...
    for ax in axes[len(reward_keys):]:
        ax.axis('off')
    ...
    # Also plot total reward. Older logs may miss newer reward components.
    total_keys = [key for key, _, _ in reward_keys]
    available_total_keys = [k for k in total_keys if k in metrics and len(metrics[k]) > 0]
    if available_total_keys:
        min_len = min(len(metrics[k]) for k in available_total_keys)
        total = np.zeros(min_len, dtype=float)
        for key in available_total_keys:
            total += np.asarray(metrics[key][:min_len], dtype=float)

        missing_total_keys = [key for key in total_keys if key not in available_total_keys]
        if missing_total_keys:
            print(
                "[Visualize] Missing reward components for total reward plot: "
                + ", ".join(missing_total_keys)

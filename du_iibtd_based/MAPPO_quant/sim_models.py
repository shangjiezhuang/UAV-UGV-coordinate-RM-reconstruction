...
class SimDataGen:
    ...
    def _prepare_radioseer_field(
        ...
        """Decode RadioSeer gain_DPM directly into normalized pixel space for MAPPO."""
    ...
    def _generate_radioseer_data(self, seed: int) -> Dict:
        ...
        arrays_rel_path = row.get("arrays_npz") or metadata.get("data_files", {}).get("arrays_npz")
        ...
        with np.load(npz_path) as arrays:
            if "S" in arrays and "Phi" in arrays:
                field_xy = np.asarray(arrays["S"], dtype=np.float64)
                phi = np.asarray(arrays["Phi"], dtype=np.float64).reshape(-1)
                self.K = int(phi.shape[0])
                self.config.scene.total_freq_bands_nums = self.K
                ground_truth = (field_xy[:, :, np.newaxis] * phi[np.newaxis, np.newaxis, :]).astype(float)
                building_key = "building_mask_xy" if "building_mask_xy" in arrays else "building_mask"
                non_building_key = (
                    "non_building_mask_xy"
                    if "non_building_mask_xy" in arrays
                    else "non_building_mask"
                )
                building_mask_xy = np.asarray(arrays[building_key], dtype=bool)
                non_building_mask_xy = np.asarray(arrays[non_building_key], dtype=bool)
            else:
        ...
        if building_mask_xy.shape != non_building_mask_xy.shape:
            raise ValueError(
                "RadioSeer building/non-building masks must share the same shape, got "
                f"{building_mask_xy.shape} vs {non_building_mask_xy.shape}"
            )
        if np.any(building_mask_xy & non_building_mask_xy):
            raise ValueError("RadioSeer building_mask and non_building_mask overlap.")
        if not np.all(building_mask_xy | non_building_mask_xy):
            raise ValueError(
                "RadioSeer building_mask and non_building_mask do not cover the full grid."
            )
...
class IIBTD_opt:
    ...
    def __init__(
        ...
        self._ensemble_member_stream_rngs: List[np.random.Generator] = []
    ...
    def reconstruct(self) -> RadioMapState:
        if self._pending_samples:
            ...
            keep_ratio = adaptive_keep_ratio(
                ...
                early_ratio=float(self.config.planner.ensemble_keep_ratio),
            ...
            if not self._ensemble_member_models:
                full_keep_ratio = 1.0 if self._reconstruct_round == 1 else keep_ratio
                ...
                mean_map, var_map, _, ensemble_info = incremental_refresh_ensemble_models(
                    ...
                    member_stream_rngs=self._ensemble_member_stream_rngs,
                    ...
                    member_nus=self._latest_ensemble_info.get("member_nus"),
            ...
            next_member_stream_rngs = list(
                ensemble_info.get("member_stream_rngs", self._ensemble_member_stream_rngs)
            )
            ...
            self._ensemble_member_stream_rngs = next_member_stream_rngs
    ...
    def reset(self) -> None:
        ...
        self._ensemble_member_stream_rngs = []
        self._ensemble_member_observation_counts = np.empty((0,), dtype=int)

...
- UAV moves only on integer grid points; UGV position is fixed.
...
- UGV keeps a stay-only action for compatibility.
"""
        ...
        while True:
            ...
            try:
                if cmd == "reset":
                    ...
                    uav_action, reset_seed = payload
                    obs, rew, term, trunc, info = env.step(int(uav_action))
                    if term or trunc:
...
class UAVUGVEnvironment:
    ...
    def step(
        ...
        # 2) UAV movement execution; UGV position remains fixed.
...
class VecUAVUGVEnvironment(_VecResetSeedMixin):
    ...
    def step(
        ...
        results = []
        for i, env in enumerate(self.envs):
            obs, rew, term, trunc, info = env.step(int(uav_actions[i]))
            if term or trunc:
                info["terminal_obs"] = obs
                obs, _ = env.reset(seed=self._next_reset_seed(i))

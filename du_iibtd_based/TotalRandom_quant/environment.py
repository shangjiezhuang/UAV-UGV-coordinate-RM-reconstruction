...
def _build_subproc_env(
    ...
    sim_data = SimDataGen(
        config=config,
        seed=int(config.run.seed + env_idx),
...
def _subproc_env_worker(
    ...
    try:
        _seed_subproc_worker_rngs(int(config.run.seed + env_idx))
...
class UAVUGVEnvironment:
    ...
    def __init__(
        ...
        self.current_quant_choice_idx = (
            int(default_quant_matches[0]) if default_quant_matches.size else 0
        )
    ...
    def _init_reset_rng(self, seed: Optional[int]) -> None:
        if seed is not None:
            ...
            local_seed = int(self.config.run.seed + self._reset_counter)
    ...
    def reset(self, seed: Optional[int] = None) -> Tuple[Dict[str, np.ndarray], dict]:
        ...
        info = {
            ...
            "current_quant_norm": float(self._current_quant_norm()),
    ...
    def _resolve_prefill_budget_basis(self) -> int:
        ...
        if basis <= 0:
            basis = int(self.config.run.episode_max_steps)
        return max(1, basis)
    ...
    def step(
        ...
        # 4) UAV samples the current grid point from noisy GT, then applies
        # log-domain quantize/dequantize at the selected bit depth.
        ...
        should_force_flush = (
            ...
            and (
                self.uav_energy <= 0
                or self.current_step >= self.config.run.episode_max_steps
            )
        )
        ...
        reached_step_limit = bool(self.current_step >= self.config.run.episode_max_steps)
    ...
    def _current_sample_packet_bits(self) -> float:
        ...
        return (
            ...
            * self.config.comm.data_per_sample
            * bit_scale
        )
            ...
            if cached_outputs is not None:
                ...
                try:
                    mean_map, var_map, _ = ensemble_reconstruct_maps(
                        ...
                        seed=self.config.run.seed + int(seed_offset),
    ...
    def _build_uav_obs(
        ...
        parts.append(np.array([self._current_quant_norm()], dtype=float))
...
class VecUAVUGVEnvironment(_VecResetSeedMixin):
    ...
    def __init__(
        ...
        self.base_seed = int(config.run.seed)

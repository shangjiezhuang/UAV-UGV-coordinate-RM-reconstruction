"""
Visualization module for IPPO training results.
...
def _occupancy_from_metrics(all_metrics: Dict[str, object]) -> Optional[np.ndarray]:
    ...
    try:
        ...
        sim_data = SimDataGen(config=config, seed=int(config.ippo.seed))
...
def plot_reward_components(
    ...
    plot_specs = [
        ...
        ('r_ugv_building_clearance', 'UGV Building Penalty', '#c0392b'),
...
def _step_reward_specs() -> List[Tuple[str, str, str, object, str]]:
    return [
        ...
        ("r_ugv_building_clearance", "UGV Building", '#c0392b', (0, (6, 1)), 'h'),
...
def plot_eval_step_details(
    ...
    # 3. Bandwidth ratio / quantization bit depth
    ax = axes[1, 0]
    data = _series("bw_ratio")
    has_bw_quant_legend = False
    if data.size > 0:
        steps = _step_axis(data.size)
        ax.step(steps, data, 'r-', linewidth=1.5, where='mid', label='BW Ratio')
        ax.set_ylim(-0.05, 1.05)
        has_bw_quant_legend = True
    ax.set_title('UAV Bandwidth / Quantization', fontsize=12, fontweight='bold')
    ax.set_xlabel('Step')
    ...
    if quant_bits.size > 0:
        steps = _step_axis(quant_bits.size)
        ax_quant = ax.twinx()
        ax_quant.step(
            steps,
            quant_bits,
        ...
        ax_quant.set_ylabel('Quant Bits')
        ax_quant.set_ylim(0, max(float(np.nanmax(quant_bits)) + 1.0, 1.0))
        has_bw_quant_legend = True
        lines1, labels1 = ax.get_legend_handles_labels()
        lines2, labels2 = ax_quant.get_legend_handles_labels()
        ax.legend(lines1 + lines2, labels1 + labels2, loc='best', fontsize=8.5)
    elif has_bw_quant_legend:

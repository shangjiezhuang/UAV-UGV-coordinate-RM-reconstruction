...
from visualize import plot_planner_trajectory_2d, plot_planner_trajectory_2d_gif
...
def render_one(
    ...
    config = Config()
    config.ippo.device = device
    if max_steps is None:
        max_steps = int(config.ippo.episode_max_steps)

    env = make_env_factory(config)()
    try:
        policy = IPPOPolicy(env.get_obs_dims(), env.get_action_dims(), config.ippo)

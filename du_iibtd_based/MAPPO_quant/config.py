...
class PlannerConfig:
    ...
    # Ensemble resampling
    ensemble_size: int = 3 
    ensemble_keep_ratio: float = 1.0
    ensemble_keep_recent: int = 3
    ensemble_quality_weighted: bool = True         # Weight ensemble members by observed-entry NMSE
    ensemble_observation_mode: str = "shared_all"  # shared_all | member_streaming_subset
...
class Config:
    ...
    def __post_init__(self):
        ...
        self.planner.ensemble_observation_mode = (
            str(self.planner.ensemble_observation_mode).strip().lower() or "shared_all"
        )
        ...
        _ensure(
            self.planner.ensemble_observation_mode in {"shared_all", "member_streaming_subset"},
            "planner.ensemble_observation_mode must be shared_all or member_streaming_subset, got "
            f"{self.planner.ensemble_observation_mode!r}",
        )
        _ensure(

...
def _preferred_train_cuda_index(cuda_count: int) -> int:
    return _pick_visible_cuda_index(cuda_count, preferred_indices=[2, 1, 0])
...
def train(config: Config, cli_args: Optional[argparse.Namespace] = None):
    ...
    if mc.eval_interval > 0:
        if int(mc.eval_seed_stride) == 0:
            print(f"[Eval] Using fixed reset seeds from base {eval_config.mappo.seed}")
        else:
            print(
                f"[Eval] Using reset seed base {eval_config.mappo.seed} "

        ...
        for _ in range(self.config.num_epochs):
            for batch in buffer.get_batches(self.config.num_minibatches, self.device):
                ...
                (
                    ...
                    uav_move_entropy,
                    uav_bw_entropy,
                ) = self.policy.uav_actor.evaluate_action(
                    batch["uav_obs"],
                    batch["uav_move_actions"],
                    batch["uav_bw_actions"],
                    batch["uav_move_action_masks"],
                    batch["uav_bw_action_masks"],
                )
                uav_new_logp = uav_move_logp + uav_bw_logp
                uav_entropy = uav_move_entropy + uav_bw_entropy

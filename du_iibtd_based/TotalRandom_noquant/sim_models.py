"""
Simulation data models for random active sensing.
"""
...
class SimDataGen:
    ...
    def reset_rng(self, seed: int) -> None:
        ...
        This is useful for seeded evaluation episodes where `env.reset(seed=...)`
        is expected to make the full run reproducible.
        """
        self.seed = int(seed)
        ...
        if self._pending_samples:
            ...
            if not self._ensemble_member_models:
                mean_map, var_map, _, ensemble_info = ensemble_reconstruct_maps(
                    ...
                    seed=self.config.run.seed + 31 * self._reconstruct_round + len(self._all_samples),
            ...
            if (
                ...
                if nmse_degradation >= nmse_refresh_delta:
                    ...
                    mean_map, var_map, _, ensemble_info = ensemble_reconstruct_maps(
                        ...
                        i_mask=self.I_mask,
                        m_ens=self.config.planner.ensemble_size,
                        seed=self.config.run.seed + 61_000 + 53 * self._reconstruct_round + len(self._all_samples),
                        member_max_iter=select_reconstruction_outer_iters(
                            fused_effective_count,
                            warmstart=False,
                        ),

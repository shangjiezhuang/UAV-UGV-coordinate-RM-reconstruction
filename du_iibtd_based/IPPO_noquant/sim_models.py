"""
Simulation data models for IPPO active sensing.
"""
...
class SimDataGen:
    ...
    def _prepare_radioseer_field(
        ...
        """Decode RadioSeer gain_DPM directly into normalized pixel space for IPPO."""
        ...
        if self._pending_samples:
            ...
            if not self._ensemble_member_models:
                mean_map, var_map, _, ensemble_info = ensemble_reconstruct_maps(
                    ...
                    seed=self.config.ippo.seed + 31 * self._reconstruct_round + len(self._all_samples),
            ...
            if (
                ...
                if nmse_degradation >= nmse_refresh_delta:
                    ...
                    mean_map, var_map, _, ensemble_info = ensemble_reconstruct_maps(
                        ...
                        i_mask=self.I_mask,
                        m_ens=self.config.planner.ensemble_size,
                        seed=self.config.ippo.seed + 61_000 + 53 * self._reconstruct_round + len(self._all_samples),
                        member_max_iter=select_reconstruction_outer_iters(
                            fused_effective_count,
                            warmstart=False,
                        ),

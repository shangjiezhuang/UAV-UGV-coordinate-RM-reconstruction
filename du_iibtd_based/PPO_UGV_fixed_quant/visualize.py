...
Provides plotting functions for:
  1. UAV trajectory and fixed UGV position
...
def _occupancy_from_metrics(all_metrics: Dict[str, object]) -> Optional[np.ndarray]:
    ...
    try:
        ...
        sim_data = SimDataGen(config=config, seed=int(config.mappo.seed))
...
def _planner_freq_style(
    planner_xyz: np.ndarray,
    planner_valid: np.ndarray,
):
    planner_cmap = plt.get_cmap('viridis')
    if planner_valid.any():
        valid_freq = np.asarray(planner_xyz[planner_valid, 2], dtype=float)
        vmin = float(np.nanmin(valid_freq))
        vmax = float(np.nanmax(valid_freq))
        ...
        vmin = 0.0
        vmax = 1.0
    return planner_cmap, Normalize(vmin=vmin, vmax=vmax)
...
def _get_step_label_series(
    ...
    arr = np.asarray(step_details[key], dtype=object).reshape(-1)
    if arr.size < frame_count:
        raise ValueError(
            f"Step-detail series {key!r} has {arr.size} values; expected at least {frame_count}"
        )
...
def _planner_history_rgba(
    ...
    *,
    ...
    freq_values = np.asarray(freq_values, dtype=float).reshape(-1)
    if freq_values.size == 0:
        return np.empty((0, 4), dtype=float)

    rgba = np.asarray(planner_cmap(planner_norm(freq_values)), dtype=float)
    if rgba.ndim == 1:
        rgba = rgba.reshape(1, 4)

    if freq_values.size == 1:
        alpha = np.asarray([max_alpha], dtype=float)
    else:
        alpha = np.linspace(min_alpha, max_alpha, freq_values.size, dtype=float)
    rgba[:, 3] = alpha
    return rgba


def _resolve_planner_display_mode(
    target_source: object,
    effective_mode: object,
    planner_submode: object,
) -> str:
    source = str(target_source).strip().lower()
    if source == "bootstrap":
        return "bootstrap"

    for raw in (effective_mode, planner_submode):
        value = str(raw).strip().lower()
        if value in {"local", "global"}:
            return value

    if source in {"planner", "active_plan", "target"}:
        return "planner"
    return "unknown"


def _planner_mode_style(display_mode: str) -> Dict[str, str]:
    ...
    return {
        "label": str(mode).upper() if mode else "UNKNOWN",
        "marker": "o",
        "bbox_fc": "#f3f4f6",
        "bbox_ec": "#9ca3af",
        "edge": "#6b7280",
...
def plot_planner_trajectory_2d(
    ...
    planner_cmap, planner_norm = _planner_freq_style(planner_xyz, planner_valid)
    ...
    if planner_valid.any():
        ...
        ax.plot(
            planner_hist[:, 0], planner_hist[:, 1],
            color='#2ca02c', linewidth=1.2, alpha=0.22, label='Planner Path', zorder=3,
        )
        ax.scatter(
            planner_hist[:, 0], planner_hist[:, 1],
            c=planner_rgba,
    ...
    end_points = {
        "uav_end": np.array([uav_steps[-1, 0], uav_steps[-1, 1], 0.0], dtype=float),
        "ugv_end": np.array([ugv_steps[-1, 0], ugv_steps[-1, 1], 0.0], dtype=float),
    }
    if planner_valid.any():
        planner_hist = planner_xyz[planner_valid]
        start_points["planner_start"] = np.array([planner_hist[0, 0], planner_hist[0, 1], 0.0], dtype=float)
        end_points["planner_end"] = np.array([planner_hist[-1, 0], planner_hist[-1, 1], 0.0], dtype=float)
    ...
    if planner_valid.any():
        planner_hist = planner_xyz[planner_valid]
        planner_start_color = planner_cmap(planner_norm(planner_hist[0, 2]))
        planner_end_color = planner_cmap(planner_norm(planner_hist[-1, 2]))
        ax.scatter(
            [start_markers["planner_start"][0]], [start_markers["planner_start"][1]],
            s=140, c=[planner_start_color], marker='^', edgecolors='black', label='Planner Start', zorder=6,
        )
        ax.scatter(
            [end_markers["planner_end"][0]], [end_markers["planner_end"][1]],
            s=120, c=[planner_end_color], marker='v', edgecolors='black', label='Planner End', zorder=6,
        )
...
def plot_planner_trajectory_2d_gif(
    ...
    target_source = _get_step_label_series(
        step_details,
        frame_count,
        "target_source",
    )
    planner_submode = _get_step_label_series(
        step_details,
        frame_count,
        "planner_submode",
    )
    planner_effective_target_mode = _get_step_label_series(
        step_details,
        frame_count,
        "planner_effective_target_mode",
    )
    planner_mode_switch = _get_step_label_series(
        step_details,
        frame_count,
        "planner_mode_switch",
    )
    planner_mode_switch_reason = _get_step_label_series(
        step_details,
        frame_count,
    ...
    planner_global_steps_remaining = _get_step_series(
        step_details,
        frame_count,
        "planner_global_steps_remaining",
    )
    planner_local_candidate_count = _get_step_series(
        step_details,
        frame_count,
        "planner_local_candidate_count",
    )
    planner_local_candidate_threshold = _get_step_series(
        step_details,
        frame_count,
        "planner_local_candidate_threshold",
    )
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    ...
    _draw_building_layout(ax, occupancy, grid_size)
    _setup_planar_axes(ax, grid_size, title)
    fig.subplots_adjust(left=0.07, right=0.80, top=0.89, bottom=0.20)
    ...
    planner_curr, = ax.plot([], [], color='#2ca02c', marker='^', markersize=12, linestyle='None', markeredgecolor='black', label='Planner Current', zorder=6)
    ...
    mode_text = ax.text(
        ...
        bbox=dict(boxstyle='round,pad=0.28', fc='white', ec='#9ca3af', alpha=0.92),
        zorder=10,
    )
    ...
    legend_handles = [
        ...
        Line2D([], [], color='#2ca02c', marker='o', markersize=8, linestyle='None', markeredgecolor='none', alpha=0.5, label='Planner Target History'),
        ...
        Line2D([], [], color='#2ca02c', marker='^', markersize=10, linestyle='None', markeredgecolor='black', label='Planner Current'),
    ]
    ax.legend(
    ...
    def _clear_scatter(scatter) -> None:
        ...
        scatter.set_facecolors(np.empty((0, 4), dtype=float))
        scatter.set_sizes(np.asarray([], dtype=float))
    ...
    def _update(frame_idx: int):
        ...
        if valid_history.any():
            ...
            planner_hist.set_offsets(hist[:, :2])
            planner_hist.set_facecolors(
                _planner_history_rgba(
                    hist[:, 2],
                    planner_cmap,
                    planner_norm,
                    min_alpha=0.08,
            ...
            planner_hist.set_edgecolors('none')
            planner_hist.set_sizes(np.full(hist.shape[0], 60.0, dtype=float))
        else:
        ...
        if planner_valid[frame_idx]:
            ...
            display_mode = _resolve_planner_display_mode(
                target_source[frame_idx],
                planner_effective_target_mode[frame_idx],
                planner_submode[frame_idx],
            )
            mode_style = _planner_mode_style(display_mode)
            planner_color = planner_cmap(planner_norm(planner_point[2]))
            planner_curr.set_marker(mode_style["marker"])
            planner_curr.set_color(planner_color)
            planner_curr.set_markerfacecolor(planner_color)
            planner_curr.set_markeredgecolor(mode_style["edge"])
        ...
        switch_label = str(planner_mode_switch[frame_idx]).strip()
        switch_reason = str(planner_mode_switch_reason[frame_idx]).strip()
        switch_text = "Switch=-"
        if switch_label or switch_reason:
            switch_text = f"Switch={switch_label or '-'}"
            if switch_reason:
                switch_text += f" ({switch_reason})"
        mode_text.set_text(
            f"Mode: {mode_style['label']}\n"
            f"Submode={str(planner_submode[frame_idx]).strip() or '-'}  "
            f"GlobalLeft={_format_optional_int(planner_global_steps_remaining[frame_idx])}\n"
            f"LocalCand={_format_optional_int(planner_local_candidate_count[frame_idx])}/"
            f"{_format_optional_int(planner_local_candidate_threshold[frame_idx])}  "
            f"{switch_text}"
        )
        mode_bbox = mode_text.get_bbox_patch()
        mode_bbox.set_facecolor(mode_style["bbox_fc"])
        mode_bbox.set_edgecolor(mode_style["bbox_ec"])
        ...
        return (
            ...
            uav_curr, ugv_curr, planner_curr, step_text, status_text, sensing_text, mode_text,
        )
...
def plot_eval_step_details(
    ...
    # 3. Bandwidth ratio + quantization bits
    ax = axes[1, 0]
    ...
    if data.size > 0:
        ...
        ratio_labels.append('BW Ratio')
        ax.set_ylim(-0.05, 1.05)
    ax.set_title('UAV Bandwidth Ratio / Quantization Bits', fontsize=12, fontweight='bold')
    ax.set_xlabel('Step')
    ax.set_ylabel('Sensing / Total BW', color='#e74c3c')
    ax.tick_params(axis='y', labelcolor='#e74c3c')
    ax.grid(True, alpha=0.3)

    quant_bits = _series("quant_bits")
    quant_choice_idx = _series("uav_quant_choice_idx")
    ax_bits = ax.twinx()
    bit_lines = []
    bit_labels = []
    if quant_bits.size > 0:
        bit_steps = _step_axis(quant_bits.size)
        line = ax_bits.step(
            bit_steps,
            quant_bits,
            color='#2c3e50',
        ...
        bit_labels.append('Quant Bits')
        finite_bits = quant_bits[np.isfinite(quant_bits)]
        if finite_bits.size > 0:
            unique_bits = np.unique(finite_bits.astype(int))
            ax_bits.set_yticks(unique_bits)
            margin = max(0.5, 0.15 * float(np.ptp(unique_bits)) if unique_bits.size > 1 else 0.5)
            ax_bits.set_ylim(float(np.min(unique_bits)) - margin, float(np.max(unique_bits)) + margin)
    elif quant_choice_idx.size > 0:
        bit_steps = _step_axis(quant_choice_idx.size)
        line = ax_bits.step(
            bit_steps,
            quant_choice_idx,
            color='#2c3e50',
            ...
            linestyle='--',
            where='mid',
            label='Quant Choice Idx',
        )[0]
        bit_lines.append(line)
        bit_labels.append('Quant Choice Idx')
    ax_bits.set_ylabel('Quant Bits', color='#2c3e50')
    ax_bits.tick_params(axis='y', labelcolor='#2c3e50')
    if ratio_lines or bit_lines:
        ax.legend(ratio_lines + bit_lines, ratio_labels + bit_labels, loc='upper right', fontsize=8.5)
...
def _get_eval_artifact_records(
    ...
    raw_uav = training_metrics.get(_prefixed_eval_history_key(prefix, "uav_trajectory"), [])
    raw_ugv = training_metrics.get(_prefixed_eval_history_key(prefix, "ugv_trajectory"), [])
    raw_step_details = training_metrics.get(_prefixed_eval_history_key(prefix, "step_details"), [])
    raw_updates = training_metrics.get(f"{prefix}_update", [])
...
def _format_final_eval_artifact_title(update_idx: Optional[int]) -> str:
    if update_idx is None:
        return "Final Eval"
    return f"Final Eval (Update {update_idx})"
...
def plot_all(log_dir: str = "logs", target_nmse: Optional[float] = None):
    ...
    eval_records = _get_eval_artifact_records(training_metrics, prefix="eval")
    if eval_records:
    ...
    final_eval_records = _get_eval_artifact_records(training_metrics, prefix="final_eval")
    ...
    if top_level_record is not None and top_level_title_suffix is not None:
        ...
        plot_eval_step_reward_components(
            ...
            title=f"Evaluation Episode - Per-Step Reward Components - {top_level_title_suffix}",
        )

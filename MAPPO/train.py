...
from utils import MetricsLogger, evaluate_policy, print_config_summary, set_seeds
...
_EVAL_UPDATE_SEED_STRIDE = 1_000
...
def parse_args():
    ...
    parser.add_argument("--rollout_length", type=int, default=defaults.mappo.rollout_length)
    ...
    parser.add_argument("--w_nmse", type=float, default=defaults.reward.w_nmse)
    parser.add_argument("--w_tx", type=float, default=defaults.reward.w_tx)
    parser.add_argument("--w_queue", type=float, default=defaults.reward.w_queue)
    parser.add_argument("--w_comm", type=float, default=defaults.reward.w_comm)
    parser.add_argument("--alpha_nmse", type=float, default=defaults.reward.alpha_nmse, help=argparse.SUPPRESS)
    parser.add_argument("--alpha_unc", type=float, default=defaults.reward.alpha_unc, help=argparse.SUPPRESS)
    parser.add_argument("--lambda_new_freq", type=float, default=defaults.reward.lambda_new_freq, help=argparse.SUPPRESS)
    parser.add_argument("--lambda_new_spatial", type=float, default=defaults.reward.lambda_new_spatial, help=argparse.SUPPRESS)
    parser.add_argument("--beta_tx", type=float, default=defaults.reward.beta_tx, help=argparse.SUPPRESS)
    parser.add_argument("--gamma_queue", type=float, default=defaults.reward.gamma_queue, help=argparse.SUPPRESS)
    parser.add_argument("--lambda_uav_progress", type=float, default=defaults.reward.lambda_uav_progress, help=argparse.SUPPRESS)
    parser.add_argument("--lambda_uav_backtrack", type=float, default=defaults.reward.lambda_uav_backtrack, help=argparse.SUPPRESS)
    parser.add_argument("--lambda_ugv_progress", type=float, default=defaults.reward.lambda_ugv_progress, help=argparse.SUPPRESS)
    parser.add_argument("--lambda_ugv_backtrack", type=float, default=defaults.reward.lambda_ugv_backtrack, help=argparse.SUPPRESS)
    parser.add_argument("--bootstrap_progress_scale", type=float, default=defaults.reward.bootstrap_progress_scale, help=argparse.SUPPRESS)
    parser.add_argument("--lambda_spatial_revisit", type=float, default=defaults.reward.lambda_spatial_revisit, help=argparse.SUPPRESS)
    parser.add_argument("--local_goal_arrival_bonus", type=float, default=defaults.reward.local_goal_arrival_bonus, help=argparse.SUPPRESS)
    parser.add_argument("--terminal_success_bonus", type=float, default=defaults.reward.terminal_success_bonus)
    ...
    parser.add_argument(
        "--adaptive_quantization_bits",
        action="store_true",
        default=defaults.planner.adaptive_quantization_bits,
        help="Let planner targets choose high/low quantization bits from uncertainty.",
    )
    parser.add_argument(
        "--no_adaptive_quantization_bits",
        action="store_false",
        dest="adaptive_quantization_bits",
        help="Disable adaptive quantization and use fixed quantization bits everywhere.",
    )
    parser.add_argument(
        "--fixed_quantization_bits",
        type=int,
        default=defaults.planner.default_quantization_bits,
        help="Fallback fixed quantization bits when adaptive quantization is disabled.",
    )
    parser.add_argument(
        "--high_quantization_bits",
        type=int,
        default=defaults.planner.high_quantization_bits,
        help="High-precision quantization used for uncertain targets.",
    )
    parser.add_argument(
        "--low_quantization_bits",
        type=int,
        default=defaults.planner.low_quantization_bits,
        help="Low-precision quantization used for less uncertain targets.",
    )
    parser.add_argument(
        "--uncertainty_quantization_threshold",
        type=float,
        default=defaults.planner.uncertainty_quantization_threshold,
        help="Normalized uncertainty threshold for switching between high/low bits.",
    )
    parser.add_argument(
        "--iibtd_backend",
        type=str,
        default=defaults.planner.iibtd_backend,
        help="II-BTD reconstruction backend: auto, cpu, or gpu.",
    )
    parser.add_argument(
        "--iibtd_device",
        type=str,
...
def build_config(args) -> Config:
    ...
    config.mappo.episode_max_steps = args.episode_max_steps
    ...
    config.reward.w_nmse = args.w_nmse
    config.reward.w_tx = args.w_tx
    config.reward.w_queue = args.w_queue
    config.reward.w_comm = args.w_comm
    if (
        float(args.w_nmse) == float(defaults.reward.w_nmse)
        and float(args.alpha_nmse) != float(defaults.reward.alpha_nmse)
    ):
        config.reward.w_nmse = float(args.alpha_nmse)
    if (
        float(args.w_queue) == float(defaults.reward.w_queue)
        and float(args.gamma_queue) != float(defaults.reward.gamma_queue)
    ):
        config.reward.w_queue = float(args.gamma_queue)
    config.reward.lambda_uav_progress = float(args.lambda_uav_progress)
    config.reward.lambda_uav_backtrack = float(args.lambda_uav_backtrack)
    config.reward.lambda_ugv_progress = float(args.lambda_ugv_progress)
    config.reward.lambda_ugv_backtrack = float(args.lambda_ugv_backtrack)
    config.reward.bootstrap_progress_scale = float(args.bootstrap_progress_scale)
    config.reward.local_goal_arrival_bonus = float(args.local_goal_arrival_bonus)
    config.reward.terminal_success_bonus = args.terminal_success_bonus
    ...
    config.planner.adaptive_quantization_bits = bool(args.adaptive_quantization_bits)
    config.planner.default_quantization_bits = max(1, int(args.fixed_quantization_bits))
    config.planner.high_quantization_bits = max(1, int(args.high_quantization_bits))
    config.planner.low_quantization_bits = max(1, int(args.low_quantization_bits))
    config.planner.uncertainty_quantization_threshold = float(
        args.uncertainty_quantization_threshold
    )
...
def make_env_factory(config: Config, data_seed: Optional[int] = None):
    ...
    shared_data_model = SimDataGen(config, seed=shared_data_seed)
    shared_data = shared_data_model.export_data()
...
def compute_timeout_bootstrap_values(
    ...
    for env_idx, info in enumerate(infos):
        ...
        timeout_state = info.get("timeout_bootstrap_critic_state")
        if timeout_state is None:
            terminal_obs = info.get("terminal_obs")
            if isinstance(terminal_obs, dict):
                timeout_state = terminal_obs.get("critic_state")

        if timeout_state is None:
            raise RuntimeError(
                "Truncated episode is missing timeout bootstrap state. "
                f"env_idx={env_idx}, info_keys={sorted(info.keys())}"
            )

        timeout_indices.append(env_idx)
        timeout_states.append(np.asarray(timeout_state, dtype=np.float32))
...
def print_progress(
    ...
    line = (
        ...
        f"ugv_pi {update_metrics['ugv_policy_loss']:.3f} | "
...
def _preferred_train_cuda_index(cuda_count: int) -> int:
    return _pick_visible_cuda_index(cuda_count, preferred_indices=[1, 0, 2])
...
def _apply_runtime_backend_policy(config: Config) -> Optional[str]:
    ...
    - The current subproc vector env uses fork, so any GPU II-BTD backend must run
      with sync rollout to avoid CUDA initialization inside forked workers.
    """
    ...
    if str(config.mappo.vec_backend).strip().lower() == "subproc":
        config.mappo.vec_backend = "sync"
        messages.insert(
            0,
            f"MAPPO uses {train_device} and II-BTD uses {iibtd_device}; "
            "switching vec_backend from 'subproc' to 'sync' because the current "
            "subproc env uses fork and GPU II-BTD cannot initialize CUDA in forked workers"
        )
...
def train(config: Config, cli_args: Optional[argparse.Namespace] = None):
    ...
    eval_env_factory = make_env_factory(
        eval_config,
        data_seed=int(config.mappo.seed + _EVAL_DATA_SEED_OFFSET),
    )
        ...
        for update in range(1, total_updates + 1):
            ...
            for _ in range(mc.rollout_length):
                ...
                action_data = policy.get_actions(
                    ...
                    uav_move_action_mask=obs["uav_move_action_mask"],
                    uav_bw_action_mask=obs["uav_bw_action_mask"],
                    ugv_action_mask=obs["ugv_action_mask"],
                )

                # 运行环境交互，获取 观测 奖励 等信息
                next_obs, rewards, terminateds, truncateds, infos = vec_env.step(
                    uav_move_actions=action_data["uav_move_action"],
                    uav_bw_actions=action_data["uav_bw_action"],
                ...
                buffer.add(
                    ...
                    ugv_obs=obs["ugv_obs"],
                    critic_state=obs["critic_state"],
                    uav_move_action=action_data["uav_move_action"],
                    uav_bw_action=action_data["uav_bw_action"],
            ...
            if mc.eval_interval > 0 and update % mc.eval_interval == 0:
                eval_seed_base = int(eval_config.mappo.seed + update * _EVAL_UPDATE_SEED_STRIDE)

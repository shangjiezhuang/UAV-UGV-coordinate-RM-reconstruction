...
class SimDataGen:
    ...
    def _quantize_with_dataset_meta(
        self,
        values: np.ndarray,
        ...
        q_bits = int(self.quant_meta.get("n_bits", 8)) if n_bits is None else int(n_bits)
        dq, _ = quantize_to_nbit(
            arr,
            n_bits=q_bits,
            value_min=v_min,
            value_max=v_max,
    ...
    def get_data_at_newpos(
        ...
        if quantized:
            values = self._quantize_with_dataset_meta(values, n_bits=quantization_bits)

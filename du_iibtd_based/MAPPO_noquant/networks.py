...
class UAVFactorizedActorNetwork(nn.Module):
    ...
    def _direction_mask_from_joint_mask(
        ...
        # each valid direction enables every bandwidth choice.
        leading_shape = mask.shape[:-1]
        reshaped = mask.reshape(*leading_shape, self.num_directions, self.num_bandwidths)
    ...
    def get_action(
        ...
        direction_dist, bandwidth_dist = self.forward(obs, action_mask)
...
class MAPPOPolicy:
    ...
    def __init__(self, obs_dims: dict, action_dims: dict, config):
        ...
        # UAV Actor: factorized direction/bandwidth policy.
    ...
    def _enter_rollout_mode(self) -> tuple[bool, bool, bool] | None:
    ...
    def _exit_rollout_mode(self, previous_modes: tuple[bool, bool, bool] | None) -> None:
        if previous_modes is None:
            return
        self.rollout_uav_actor.train(previous_modes[0])
        self.rollout_ugv_actor.train(previous_modes[1])
        self.rollout_critic.train(previous_modes[2])

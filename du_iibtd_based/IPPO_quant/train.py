"""
Main training script for UAV-UGV Cooperative IPPO.
"""
...
@dataclass
class BestEvalCheckpoints:
    nmse: float = float("inf")
    nmse_update: Optional[int] = None
    nmse_path: Optional[str] = None
    mean_return: float = float("-inf")
    mean_return_update: Optional[int] = None
    mean_return_path: Optional[str] = None
...
def parse_args():
    parser = argparse.ArgumentParser(description="IPPO UAV-UGV Training")
    defaults = Config()

    parser.add_argument("--num_envs", type=int, default=defaults.ippo.num_envs)
    parser.add_argument("--total_timesteps", type=int, default=defaults.ippo.total_timesteps)
    parser.add_argument(
        "--rollout_length",
        type=int,
        default=None,
    ...
    parser.add_argument("--episode_max_steps", type=int, default=defaults.ippo.episode_max_steps)
    parser.add_argument("--num_minibatches", type=int, default=defaults.ippo.num_minibatches)
    parser.add_argument("--num_epochs", type=int, default=defaults.ippo.num_epochs)
    parser.add_argument("--lr_actor", type=float, default=defaults.ippo.lr_actor)
    parser.add_argument("--lr_critic", type=float, default=defaults.ippo.lr_critic)
    parser.add_argument("--seed", type=int, default=defaults.ippo.seed)
    parser.add_argument("--device", type=str, default=defaults.ippo.device)
    parser.add_argument("--vec_backend", type=str, default=defaults.ippo.vec_backend)
    ...
    parser.add_argument("--lambda_ugv_building_clearance", type=float, default=defaults.reward.lambda_ugv_building_clearance)
    parser.add_argument("--ugv_building_safe_clearance", type=int, default=defaults.reward.ugv_building_safe_clearance)
    ...
    parser.add_argument(
        "--acquisition_mode",
        type=str,
        default=defaults.planner.acquisition_mode,
        help="Planner acquisition scoring: uncertainty or uncertainty_distance.",
    )
    parser.add_argument(
        "--acquisition_distance_weight",
        type=float,
        default=defaults.planner.lambda_c,
        help="Distance penalty weight used when acquisition_mode=uncertainty_distance.",
    )
    parser.add_argument(
        "--acquisition_distance_mode",
        type=str,
        default=defaults.planner.distance_cost_mode,
        help="Distance metric for acquisition distance cost: manhattan or euclidean.",
    )
    parser.add_argument(
        "--iibtd_mu",
        type=float,
    ...
    parser.add_argument("--log_dir", type=str, default=defaults.ippo.log_dir)
    parser.add_argument("--model_dir", type=str, default=defaults.ippo.model_dir)
    parser.add_argument("--log_interval", type=int, default=defaults.ippo.log_interval)
    parser.add_argument("--eval_interval", type=int, default=defaults.ippo.eval_interval)
    parser.add_argument("--eval_episodes", type=int, default=defaults.ippo.eval_episodes)
    parser.add_argument("--eval_seed_stride", type=int, default=defaults.ippo.eval_seed_stride)
    parser.add_argument("--save_interval", type=int, default=defaults.ippo.save_interval)
...
def build_config(args) -> Config:
    ...
    config.ippo.num_envs = args.num_envs
    config.ippo.total_timesteps = args.total_timesteps
    config.ippo.episode_max_steps = int(args.episode_max_steps)
    config.ippo.rollout_length = config.ippo.episode_max_steps
    config.ippo.num_minibatches = args.num_minibatches
    config.ippo.num_epochs = args.num_epochs
    config.ippo.lr_actor = args.lr_actor
    config.ippo.lr_critic = args.lr_critic
    config.ippo.seed = args.seed
    config.ippo.device = args.device
    config.ippo.vec_backend = str(args.vec_backend).strip().lower()
    config.ippo.log_dir = args.log_dir
    config.ippo.model_dir = args.model_dir
    config.ippo.log_interval = args.log_interval
    config.ippo.eval_interval = args.eval_interval
    config.ippo.eval_episodes = max(1, int(args.eval_episodes))
    config.ippo.eval_seed_stride = max(0, int(args.eval_seed_stride))
    config.ippo.save_interval = args.save_interval
    ...
    config.reward.lambda_ugv_building_clearance = max(
        0.0,
        float(args.lambda_ugv_building_clearance),
    )
    config.reward.ugv_building_safe_clearance = max(
        1,
        int(args.ugv_building_safe_clearance),
    )
    ...
    config.planner.acquisition_mode = str(args.acquisition_mode).strip().lower()
    config.planner.lambda_c = max(0.0, float(args.acquisition_distance_weight))
    config.planner.distance_cost_mode = (
        str(args.acquisition_distance_mode).strip().lower()
        or config.planner.distance_cost_mode
    )
...
def make_env_factory(
    ...
    if shared_data is None:
        shared_data_seed = int(config.ippo.seed if data_seed is None else data_seed)
...
def make_vector_env(
    ...
    if config.ippo.vec_backend == "subproc":
        return SubprocVecUAVUGVEnvironment(
            num_envs=config.ippo.num_envs,
            config=config,
            shared_data=shared_data,
            minimal_info=minimal_info,
        )
    return VecUAVUGVEnvironment(
        num_envs=config.ippo.num_envs,
        config=config,
...
def make_episode_trackers(
    ...
    """Create per-env episode metric accumulators."""
    return [{key: 0.0 for key in keys} for _ in range(num_envs)], [0] * num_envs
...
def reset_episode_tracker_entry(
    ...
    accumulators[env_idx] = {key: 0.0 for key in keys}
...
def compute_timeout_bootstrap_values(
    ...
    uav_timeout_values = np.zeros(len(infos), dtype=np.float32)
    ugv_timeout_values = np.zeros(len(infos), dtype=np.float32)
    timeout_indices: List[int] = []
    timeout_uav_obs: List[np.ndarray] = []
    timeout_ugv_obs: List[np.ndarray] = []
    missing_terminal_obs: List[int] = []
    ...
    for env_idx, info in enumerate(infos):
        ...
        if (
            not isinstance(terminal_obs, dict)
            or "uav_obs" not in terminal_obs
            or "ugv_obs" not in terminal_obs
        ):
            missing_terminal_obs.append(int(env_idx))
            continue
    ...
    if missing_terminal_obs:
        raise RuntimeError(
            "Time-limit truncation requires terminal_obs for bootstrap values; "
            f"missing terminal_obs for env indices {missing_terminal_obs}."
        )

    if timeout_indices:
        timeout_values = policy.get_values(
            uav_obs=np.stack(timeout_uav_obs, axis=0),
            ugv_obs=np.stack(timeout_ugv_obs, axis=0),
        )
        index = np.asarray(timeout_indices, dtype=int)
        uav_timeout_values[index] = np.asarray(timeout_values["uav_value"], dtype=np.float32)
        ugv_timeout_values[index] = np.asarray(timeout_values["ugv_value"], dtype=np.float32)

    return {
        "uav_timeout_value": uav_timeout_values,
        "ugv_timeout_value": ugv_timeout_values,
    }
...
def eval_seed_base(eval_config: Config, update: int, eval_seed_stride: int) -> int:
    return int(eval_config.ippo.seed + int(update) * max(int(eval_seed_stride), 0))
...
def run_evaluation(
    ...
    return evaluate_scene_suite(
        envs=eval_envs,
        evaluate_fn=evaluate_policy,
        policy=policy,
        num_episodes=config.ippo.eval_episodes,
        max_steps=config.ippo.episode_max_steps,
        seed_base=eval_seed_base(eval_config, update, config.ippo.eval_seed_stride),
    )
...
def _resolve_iibtd_runtime_device(config: Config) -> str:
    ...
    if not device or device.lower() == "auto":
        return _normalize_device_string(config.ippo.device)
...
def _apply_runtime_backend_policy(config: Config) -> Optional[str]:
    ...
    - Prefer IPPO on cuda:1 and II-BTD on cuda:2 when those devices exist.
    - If fewer GPUs are available, fall back to visible devices while still trying
      to keep IPPO and II-BTD on different GPUs.
    ...
    train_device_arg = str(config.ippo.device).strip().lower()
    ...
    if not train_device_arg or train_device_arg == "cuda":
        config.ippo.device = _normalize_device_string(
            config.ippo.device,
    ...
    if train_device != iibtd_device:
        messages.insert(
            0,
            f"IPPO uses {train_device} and II-BTD uses {iibtd_device}"
        )
        return "; ".join(messages)

    messages.insert(
        0,
        f"IPPO training and II-BTD both use {train_device}"
    )
    return "; ".join(messages)
...
def train(config: Config, cli_args: Optional[argparse.Namespace] = None):
    ...
    eval_config.ippo.seed = int(config.ippo.seed + _EVAL_RESET_SEED_OFFSET)
    ...
    try:
        ...
        if mc.eval_interval > 0:
            ...
            if int(mc.eval_seed_stride) == 0:
                print(
                    ...
                    f"{eval_config.ippo.seed} so starts/prefill can differ."
                )
            else:
                print(
                    f"[Eval] Reusing training DPM100PSD scene suite "
                    f"{format_scene_suite(scene_indices)}; reset seed base "
                    f"{eval_config.ippo.seed} with stride {int(mc.eval_seed_stride)}."
                )
        ...
        print(
            "[Runtime Devices] "
            f"IPPO train/update={policy.device} | "
        ...
        global_step = 0
        best_eval = BestEvalCheckpoints()
        last_eval_results: Optional[Dict[str, float]] = None
        last_eval_update: Optional[int] = None

        print(f"\nStarting training for {total_updates} updates ({total_transitions:,} executed transitions)")
        ...
        for update in range(1, total_updates + 1):
            ...
            for _ in range(mc.rollout_length):
                # 获取当前policy下的动作以及每个agent的本地critic value
                ...
                buffer.add(
                    ...
                    uav_action_mask=obs["uav_action_mask"],
                    ugv_action_mask=obs["ugv_action_mask"],
                    uav_reward=rewards["uav_reward"],
                    ugv_reward=rewards["ugv_reward"],
            ...
            # rollout结束后计算每个agent的本地critic bootstrap value
            ...
            # 利用前述数据训练IPPO 更新策略
            update_metrics = ippo.update(buffer)
            ...
            if mc.eval_interval > 0 and update % mc.eval_interval == 0:
                if not eval_envs:
                    raise RuntimeError("eval_interval > 0 but eval envs were not initialized.")
        ...
        if mc.eval_interval > 0:
            final_eval_seed_base = eval_seed_base(
                eval_config,
                total_updates,
                mc.eval_seed_stride,
            )
            if last_eval_results is not None and last_eval_update == int(total_updates):
                ...
                if not eval_envs:
                    raise RuntimeError("eval_interval > 0 but eval envs were not initialized.")
                final_eval_results = run_evaluation(
                    eval_envs=eval_envs,
                    policy=policy,
                    config=config,
                    eval_config=eval_config,
        ...
        if best_eval.nmse_path is not None and best_eval.nmse_update is not None:
            print(
                f"Best NMSE checkpoint saved to {best_eval.nmse_path} "
                f"(update {best_eval.nmse_update}, eval_mean_nmse={best_eval.nmse:.6f})"
            )
        if best_eval.mean_return_path is not None and best_eval.mean_return_update is not None:
            print(
                f"Best return checkpoint saved to {best_eval.mean_return_path} "
                f"(update {best_eval.mean_return_update}, "
                f"eval_mean_return={best_eval.mean_return:.6f})"
            )

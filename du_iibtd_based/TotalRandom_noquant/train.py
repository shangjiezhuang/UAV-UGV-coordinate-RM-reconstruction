"""Run the total-random UAV-UGV no-quant baseline."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Total-random UAV-UGV no-quant baseline")
    defaults = Config()
...
def build_run_metadata(config: Config, cli_args: Optional[argparse.Namespace]) -> Dict[str, object]:
    metadata: Dict[str, object] = {
        ...
        "run_info": {
            "argv": list(sys.argv),
            "cwd": os.getcwd(),
            "control_policy": "uniform_random_legal_actions",
        },
    }

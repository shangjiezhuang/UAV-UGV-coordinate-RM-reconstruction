...
def parse_args() -> argparse.Namespace:
    ...
    if hasattr(defaults.uav, "quant_bits"):
        parser.add_argument("--uav_quant_bits", type=str, default=",".join(str(bits) for bits in defaults.uav.quant_bits))
        parser.add_argument("--uav_default_quant_bits", type=int, default=defaults.uav.default_quant_bits)
...
def build_config(args: argparse.Namespace) -> Config:
    ...
    if hasattr(config.uav, "quant_bits"):
        config.uav.quant_bits = _parse_int_list(args.uav_quant_bits)
...
def main() -> None:
    ...
    print("  BW/quant selection:     uncertainty-ranked bins")
    print("-" * 60 + "\n")

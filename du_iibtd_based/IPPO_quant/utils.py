...
def evaluate_policy(
    ...
    Args:
        ...
        policy: IPPOPolicy with trained networks.
    ...
    for ep in range(num_episodes):
        ...
        step_local_unc_mean = []
        step_current_cell_unc_mean = []
        step_target_freq_unc = []
        step_var_map_age_norm = []
        step_pending_ensemble_norm = []
        step_ugv_action = []
        step_r_nmse = []
        step_r_unc = []
        step_r_new_freq = []
        ...
        for step in range(max_steps):
            ...
            step_channel_los.append(info["channel_los"])
            ...
            step_local_unc_mean.append(info["local_unc_mean"])
            step_current_cell_unc_mean.append(info["current_cell_unc_mean"])
            step_target_freq_unc.append(info["target_freq_unc"])
            step_var_map_age_norm.append(info["var_map_age_norm"])
            step_pending_ensemble_norm.append(info["pending_ensemble_norm"])
            ...
            step_r_ugv_building_clearance.append(info["r_ugv_building_clearance"])
            step_r_terminal.append(info["r_terminal"])
            step_ugv_building_clearance_norm.append(info["ugv_building_clearance_norm"])
            step_ugv_building_clearance_deficit.append(info["ugv_building_clearance_deficit"])
            ...
            ep_r_ugv_building_clearance += info["r_ugv_building_clearance"]
        ...
        all_r_ugv_building_clearance.append(ep_r_ugv_building_clearance)
        ...
        last_step_details = {
            ...
            "local_unc_mean": step_local_unc_mean,
            "current_cell_unc_mean": step_current_cell_unc_mean,
            "target_freq_unc": step_target_freq_unc,
            "var_map_age_norm": step_var_map_age_norm,
            "pending_ensemble_norm": step_pending_ensemble_norm,
            ...
            "r_ugv_building_clearance": step_r_ugv_building_clearance,
    ...
    return {
        **_eval_scene_metadata(env, seed_base),
        ...
        "eval_mean_r_ugv_building_clearance": np.mean(all_r_ugv_building_clearance),
...
def print_config_summary(config: Config):
    ...
    prefill_basis = (
        ...
        else config.ippo.episode_max_steps
    )
    ...
    print(f"  Acquisition:      {config.planner.acquisition_mode}")
    print(f"  Acquisition λ_u:  {config.planner.lambda_u}")
    print(f"  Acquisition λ_c:  {config.planner.lambda_c}")
    print(f"  Distance metric:  {config.planner.distance_cost_mode}")
    ...
    print(f"\n--- IPPO ---")
    print(f"  Envs: {config.ippo.num_envs}")
    print(f"  Vec backend: {config.ippo.vec_backend}")
    print(f"  Total steps: {config.ippo.total_timesteps:,}")
    print(f"  Rollout length: {config.ippo.rollout_length}")
    print(f"  LR (actor/critic): {config.ippo.lr_actor}/{config.ippo.lr_critic}")
    print(f"  Gamma: {config.ippo.gamma}, GAE λ: {config.ippo.gae_lambda}")
    print(f"  Eval seed stride: {config.ippo.eval_seed_stride}")
    ...
    print(f"  λ_ugv_clearance:  {config.reward.lambda_ugv_building_clearance}")
    print(f"  ugv clearance r:  {config.reward.ugv_building_safe_clearance}")

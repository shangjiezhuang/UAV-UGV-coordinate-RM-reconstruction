...
class UAVFactorizedActorNetwork(nn.Module):
    """UAV actor with a shared backbone and separate direction/bandwidth/bit heads.
    ...
    def _direction_mask_from_joint_mask(
        ...
        if joint_mask is None:
            return None
        return joint_mask.any(dim=-1).any(dim=-1)

    def _joint_mask_for_direction(
        self,
        action_mask: torch.Tensor = None,
        ...
        gather_index = direction_action.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(
            *direction_action.shape,
            1,
            self.num_bandwidths,
            self.num_quant_bits,
        )
        return joint_mask.gather(dim=-3, index=gather_index).squeeze(-3)
    ...
    def _bandwidth_mask_from_joint_mask(
        ...
        direction_mask = self._joint_mask_for_direction(action_mask, direction_action)
        if direction_mask is None:
            return None
        return direction_mask.any(dim=-1)

    def _quant_bit_mask_from_joint_mask(
        self,
        action_mask: torch.Tensor = None,
        direction_action: torch.Tensor = None,
        bandwidth_action: torch.Tensor = None,
    ) -> Optional[torch.Tensor]:
        direction_mask = self._joint_mask_for_direction(action_mask, direction_action)
        if direction_mask is None or bandwidth_action is None:
            return None

        bandwidth_action = bandwidth_action.to(
            device=direction_mask.device,
            dtype=torch.long,
        ).clamp(0, self.num_bandwidths - 1)
        gather_index = bandwidth_action.unsqueeze(-1).unsqueeze(-1).expand(
            *bandwidth_action.shape,
            1,
            self.num_quant_bits,
        )
        return direction_mask.gather(dim=-2, index=gather_index).squeeze(-2)
    ...
    def forward(
        ...
        quant_bit_mask = self._quant_bit_mask_from_joint_mask(
            action_mask,
            direction_action,
            bandwidth_action,
        )
        ...
        quant_bit_dist = self._masked_categorical(quant_bit_logits, quant_bit_mask)
    ...
    def evaluate_action(
        ...
        direction_dist, bandwidth_dist, quant_bit_dist = self.forward(
            obs,
            action_mask,

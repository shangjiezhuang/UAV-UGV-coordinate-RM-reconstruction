...
DEFAULT_ENSEMBLE_OBSERVATION_MODE = "shared_all"
...
def _normalize_score_map(
    ...
    out = np.full_like(arr, float(fill_value), dtype=float)
...
def _summarize_ensemble_models(
    ...
    *,
    ...
    member_stream_rngs: Optional[List[np.random.Generator]] = None,
    ...
    member_observation_mode: str = DEFAULT_ENSEMBLE_OBSERVATION_MODE,
    ...
    info = {
        ...
        "member_stream_rngs": list(member_stream_rngs or []),
        ...
        "ensemble_observation_mode": str(member_observation_mode),
...
def _distribute_incremental_rows_across_members(
    ...
    *,
    ...
    member_stream_rngs = list(member_stream_rngs or [])
    member_count = len(member_stream_rngs)
    if member_count == 0:
        raise RuntimeError("No member RNG state is available for streaming subset updates.")

    current_counts = np.asarray(member_observation_counts, dtype=int).copy()
    if current_counts.shape[0] != member_count:
        raise RuntimeError("member_observation_counts does not match member_stream_rngs.")
    ...
    keep_probability = float(np.clip(keep_probability, 0.0, 1.0))
    accept_mask = np.zeros((member_count, row_count), dtype=bool)
    for member_idx, rng in enumerate(member_stream_rngs):
        accept_mask[member_idx] = rng.random(row_count) < keep_probability

    for row_idx in range(row_count):
        assigned_members = np.flatnonzero(accept_mask[:, row_idx])
        if assigned_members.size == 0:
            chosen_member = int(np.argmin(current_counts))
            accept_mask[chosen_member, row_idx] = True
            assigned_members = np.array([chosen_member], dtype=int)
        current_counts[assigned_members] += 1

    member_batches: List[Optional[Dict[str, np.ndarray]]] = []
    member_accept_counts = np.sum(accept_mask, axis=1).astype(int)
    member_updated_mask = member_accept_counts > 0
    for member_idx in range(member_count):
        mask = accept_mask[member_idx]
        if not np.any(mask):
            member_batches.append(None)
            continue
        member_batches.append(
            {
                "obs_locs": new_obs_locs[mask].copy(),
                "gamma": new_gamma[mask].copy(),
                "omega": new_omega[mask].copy(),
            }
        )
...
def ensemble_reconstruct_maps(
    ...
    member_observation_mode = str(member_observation_mode or DEFAULT_ENSEMBLE_OBSERVATION_MODE).strip().lower()
    if member_observation_mode not in {"shared_all", "member_streaming_subset"}:
        raise ValueError(f"Unsupported member_observation_mode: {member_observation_mode}")

    for m in range(member_count):
        rng = np.random.default_rng(seed + 1000 + m)
        if member_observation_mode == "shared_all":
    ...
    mean_map, var_map, stack, info = _summarize_ensemble_models(
        ...
        member_stream_rngs=member_stream_rngs,
        ...
        member_observation_mode=member_observation_mode,
        fusion_meta=fusion_meta,
    )
    if return_info:
        return mean_map, var_map, stack, info
...
def incremental_refresh_ensemble_models(
    *,
    ...
    if len(member_models) != len(member_stream_rngs):
        raise RuntimeError("member_models and member_stream_rngs must have the same length.")
    ...
    elif member_observation_mode == "member_streaming_subset":
        (
            member_batches,
            updated_observation_counts,
            member_accept_counts,
            member_updated_mask,
        ) = _distribute_incremental_rows_across_members(
            new_obs_locs,
            new_gamma,
            new_omega,
            member_stream_rngs=member_stream_rngs,
...
def build_acquisition_space(
    ...
    lambda_c: float = 0.15,
    distance_cost_mode: str = "manhattan",
    distance_norm_den: Optional[float] = None,
    distance_progress_floor: Optional[float] = None,
    candidate_mask: Optional[np.ndarray] = None,
) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    """Build spatial acquisition score map."""
    var_map = np.asarray(var_map, dtype=float)
    if var_map.ndim != 3:
        raise ValueError(f"var_map must be 3-D, got shape {var_map.shape}")

    uncertainty_space = np.mean(var_map, axis=2)
    mode = str(acquisition_mode or "uncertainty").strip().lower().replace("-", "_")
    if mode in {"uncertainty+distance", "uncertainty_distance"}:
        mode = "uncertainty_distance"
    if mode not in {"uncertainty", "uncertainty_distance"}:
        raise ValueError(
            "acquisition_mode must be one of uncertainty/uncertainty_distance, "
            f"got {acquisition_mode!r}"
        )

    components: Dict[str, np.ndarray] = {"uncertainty_space": uncertainty_space}
    if mode == "uncertainty":
        acquisition_space = float(lambda_u) * uncertainty_space
        return acquisition_space, components

    nx, ny, _ = var_map.shape
    if candidate_mask is None:
        candidate_mask_arr = np.ones((nx, ny), dtype=bool)
    else:
        candidate_mask_arr = np.asarray(candidate_mask, dtype=bool)
        if candidate_mask_arr.shape != (nx, ny):
            raise ValueError(
                f"candidate_mask must have shape {(nx, ny)}, got {candidate_mask_arr.shape}"
            )
    if current_position is None:
        raise ValueError("current_position is required when acquisition_mode='uncertainty_distance'")

    current_position = np.asarray(current_position, dtype=float).reshape(2)
    xx, yy = np.meshgrid(np.arange(nx), np.arange(ny), indexing="ij")
    dx = xx.astype(float) - float(current_position[0])
    dy = yy.astype(float) - float(current_position[1])
    distance_mode = str(distance_cost_mode or "manhattan").strip().lower()
    if distance_mode == "manhattan":
        dist_move = np.abs(dx) + np.abs(dy)
    elif distance_mode == "euclidean":
        dist_move = np.sqrt(dx ** 2 + dy ** 2)
    else:
        raise ValueError(f"Unknown distance_cost_mode: {distance_cost_mode}")

    normalized_uncertainty = _normalize_score_map(
        uncertainty_space,
        mask=candidate_mask_arr,
        fill_value=0.0,
    )
    if distance_progress_floor is not None:
        progress_floor = max(float(distance_progress_floor), 1e-12)
        denom = np.maximum(dist_move, progress_floor)
        move_cost = np.zeros_like(dist_move, dtype=float)
        positive_dist = dist_move > 1e-12
        move_cost[positive_dist] = (
            np.maximum(dist_move[positive_dist] - progress_floor, 0.0)
            / denom[positive_dist]
        )
        move_cost = np.clip(move_cost, 0.0, 1.0)
        move_cost[~candidate_mask_arr] = 0.0
        distance_den = float("nan")
    elif distance_norm_den is None:
        move_cost = _normalize_score_map(
            dist_move,
            mask=candidate_mask_arr,
            fill_value=0.0,
        )
        ...
        distance_den = max(float(distance_norm_den), 1e-12)
        move_cost = np.asarray(dist_move, dtype=float) / distance_den
        move_cost = np.clip(move_cost, 0.0, 1.0)
        move_cost[~candidate_mask_arr] = 0.0
    acquisition_space = np.full((nx, ny), -np.inf, dtype=float)
    acquisition_space[candidate_mask_arr] = (
        float(lambda_u) * normalized_uncertainty[candidate_mask_arr]
        - float(lambda_c) * move_cost[candidate_mask_arr]
    )
    components.update(
        normalized_uncertainty_space=normalized_uncertainty,
        move_cost=move_cost,
        distance_norm_den=np.full((nx, ny), distance_den, dtype=float),
        distance_progress_floor=np.full(
            (nx, ny),
            float("nan") if distance_progress_floor is None else float(distance_progress_floor),
            dtype=float,
        ),

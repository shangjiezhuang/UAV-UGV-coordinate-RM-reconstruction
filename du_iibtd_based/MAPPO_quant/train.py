...
from config import Config, DU_IIBTD_BACKENDS
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Fallback DU-IIBTD nu used only when checkpoint metadata is missing.",
    )
    parser.add_argument(
        "--iibtd_kernel_bandwidth",
        type=float,
        default=defaults.planner.iibtd_kernel_bandwidth,
        help="Fallback DU-IIBTD kernel bandwidth used only when checkpoint metadata is missing.",
    )
    parser.add_argument(
        "--iibtd_backend",
        type=str,
...
def build_config(args) -> Config:
    ...
    config.mappo.eval_episodes = max(1, int(args.eval_episodes))
...
def _apply_runtime_backend_policy(config: Config) -> Optional[str]:
    ...
    - Prefer MAPPO on cuda:1 and DU-IIBTD on cuda:2 when those devices exist.
    - If fewer GPUs are available, fall back to visible devices while still trying
      to keep MAPPO and DU-IIBTD on different GPUs.
    ...
    if train_device != iibtd_device:
        messages.insert(
            0,
            f"MAPPO uses {train_device} and DU-IIBTD uses {iibtd_device}"
        )
        return "; ".join(messages)

    messages.insert(
        0,
        f"MAPPO training and DU-IIBTD both use {train_device}"
    )
    return "; ".join(messages)
    ...
    if mc.eval_interval > 0:
        if int(mc.eval_seed_stride) == 0:
            print(
                ...
                f"{eval_config.mappo.seed} so starts/prefill can differ."
            )
        else:
            print(
                f"[Eval] Reusing training DPM100PSD scene suite "
                f"{format_scene_suite(scene_indices)}; reset seed base "
                f"{eval_config.mappo.seed} with stride {int(mc.eval_seed_stride)}."
            )

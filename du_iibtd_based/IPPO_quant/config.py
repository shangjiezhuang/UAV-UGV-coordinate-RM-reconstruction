"""
Configuration for UAV-UGV Cooperative Spectrum Sensing with IPPO.
...
class RewardConfig:
    ...
    gamma_queue: float = 2                      # Weight for queue-bits penalty
    ...
    lambda_ugv_building_clearance: float = 0.0   # Penalty when UGV is too close to buildings
...
class PlannerConfig:
    ...
    # Acquisition weights
    acquisition_mode: str = "uncertainty"          # uncertainty | uncertainty_distance
    lambda_u: float = 1.0
    lambda_c: float = 0.15                         # Distance penalty weight when acquisition_mode=uncertainty_distance
    distance_cost_mode: str = "manhattan"          # Distance metric for acquisition distance cost: manhattan | euclidean
    beta_f: float = 0.3
...
@dataclass
class IPPOConfig:
    """IPPO algorithm hyperparameters."""
    # Training
...
class Config:
    ...
    def __post_init__(self):
        ...
        self.planner.acquisition_mode = (
            str(self.planner.acquisition_mode).strip().lower().replace("-", "_")
            or "uncertainty"
        )
        if self.planner.acquisition_mode in {"uncertainty+distance", "distance"}:
            self.planner.acquisition_mode = "uncertainty_distance"
        self.planner.lambda_u = float(self.planner.lambda_u)
        self.planner.lambda_c = float(self.planner.lambda_c)
        self.planner.distance_cost_mode = (
            str(self.planner.distance_cost_mode).strip().lower() or "manhattan"
        )
        self.planner.beta_f = float(self.planner.beta_f)
        _ensure(
        ...
        _ensure(
            self.planner.acquisition_mode in {"uncertainty", "uncertainty_distance"},
            "planner.acquisition_mode must be one of uncertainty/uncertainty_distance, got "
            f"{self.planner.acquisition_mode!r}",
        )
        _ensure(
            self.planner.lambda_u >= 0.0,
            f"planner.lambda_u must be non-negative, got {self.planner.lambda_u}",
        )
        _ensure(
            self.planner.lambda_c >= 0.0,
            f"planner.lambda_c must be non-negative, got {self.planner.lambda_c}",
        )
        _ensure(
            self.planner.distance_cost_mode in {"manhattan", "euclidean"},
            "planner.distance_cost_mode must be one of manhattan/euclidean, got "
            f"{self.planner.distance_cost_mode!r}",
        )
        _ensure(
        ...
        self.ippo.num_envs = int(self.ippo.num_envs)
        self.ippo.total_timesteps = int(self.ippo.total_timesteps)
        self.ippo.episode_max_steps = int(self.ippo.episode_max_steps)
        self.ippo.rollout_length = int(self.ippo.rollout_length)
        self.ippo.num_minibatches = int(self.ippo.num_minibatches)
        self.ippo.num_epochs = int(self.ippo.num_epochs)
        self.ippo.eval_episodes = int(self.ippo.eval_episodes)
        _ensure(self.ippo.num_envs > 0, f"ippo.num_envs must be positive, got {self.ippo.num_envs}")
        _ensure(
            self.ippo.total_timesteps > 0,
            f"ippo.total_timesteps must be positive, got {self.ippo.total_timesteps}",
        )
        _ensure(
            self.ippo.episode_max_steps > 0,
            f"ippo.episode_max_steps must be positive, got {self.ippo.episode_max_steps}",
        )
        _ensure(
            self.ippo.rollout_length > 0,
            f"ippo.rollout_length must be positive, got {self.ippo.rollout_length}",
        )
        _ensure(
            self.ippo.num_minibatches > 0,
            f"ippo.num_minibatches must be positive, got {self.ippo.num_minibatches}",
        )
        _ensure(
            self.ippo.num_epochs > 0,
            f"ippo.num_epochs must be positive, got {self.ippo.num_epochs}",
        )
        _ensure(
            self.ippo.eval_episodes > 0,
            f"ippo.eval_episodes must be positive, got {self.ippo.eval_episodes}",
        )

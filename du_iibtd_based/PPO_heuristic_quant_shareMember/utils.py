...
def evaluate_policy(
    ...
    for ep in range(num_episodes):
        ...
        for step in range(max_steps):
            ...
            ugv_action = int(info["ugv_action"])
                ...
                ensemble_events.append(
                    {
                        ...
                        "member_kernel_bandwidths": info["ensemble_member_kernel_bandwidths"],
                        "member_du_checkpoint_paths": info["ensemble_member_du_checkpoint_paths"],
                        "member_last_incremental_accept_counts": info["ensemble_member_last_incremental_accept_counts"],
                        "member_incremental_updated_mask": info["ensemble_member_incremental_updated_mask"],
                    }
                )
...
def print_config_summary(config: Config):
    ...
    print(f"  UGV: heuristic controller in env ({config.ugv.num_directions} legal moves)")
    ...
    print("  Ensemble members: shared-all observations, kernel bandwidth base+/-0.17")
    ...
    if config.planner.iibtd_backend in {"du_iibtd_sr", "du_iibtd_sr_learn_nu"}:
        print(f"  DU checkpoint:    {config.planner.iibtd_du_checkpoint_path}")

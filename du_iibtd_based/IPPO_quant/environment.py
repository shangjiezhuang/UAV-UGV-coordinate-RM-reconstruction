...
def _build_subproc_env(
    ...
    sim_data = SimDataGen(
        config=env_config,
        seed=int(env_config.ippo.seed + env_idx),
...
def _subproc_env_worker(
    ...
    try:
        _seed_subproc_worker_rngs(int(config.ippo.seed + env_idx))
...
class UAVUGVEnvironment:
    ...
    def _select_planner_candidates(
        ...
        *,
        ...
        distance_mode = str(self.config.planner.distance_cost_mode).strip().lower()
        acquisition_space, _ = build_acquisition_space(
            var_map=self.latest_var_map,
            lambda_u=self.config.planner.lambda_u,
            acquisition_mode=self.config.planner.acquisition_mode,
            current_position=self.uav_pos,
            lambda_c=self.config.planner.lambda_c,
            distance_cost_mode=distance_mode,
            distance_progress_floor=float(max(self.uav_step_count, 1)),
    ...
    def _init_reset_rng(self, seed: Optional[int]) -> None:
        if seed is not None:
            ...
            local_seed = int(self.config.ippo.seed + self._reset_counter)
    ...
    def _resolve_prefill_budget_basis(self) -> int:
        ...
        if basis <= 0:
            basis = int(self.config.ippo.episode_max_steps)
        return max(1, basis)
    ...
    def step(
        ...
        # 设置UAV的观测带宽和移动 
        uav_move_steps = self._apply_uav_joint_action(int(uav_action))

        # 2) Movement execution.
        ugv_move_steps = self._move_ugv(int(ugv_action))
        ...
        should_force_flush = (
            ...
            and (
                self.uav_energy <= 0
                or self.current_step >= self.config.ippo.episode_max_steps
            )
        )
        ...
        reached_step_limit = bool(self.current_step >= self.config.ippo.episode_max_steps)
        ...
        ugv_terminal_reward = 0.0
        terminal_reward = uav_terminal_reward + ugv_terminal_reward
        if abs(terminal_reward) > 0.0:
            rewards["uav_reward"] = float(rewards["uav_reward"] + uav_terminal_reward)
            rewards["ugv_reward"] = float(rewards["ugv_reward"] + ugv_terminal_reward)
            rewards["team_reward"] = float(rewards["uav_reward"] + rewards["ugv_reward"])
        reward_info["r_terminal"] = float(terminal_reward)
        reward_info["r_uav_terminal"] = float(uav_terminal_reward)
        reward_info["r_ugv_terminal"] = float(ugv_terminal_reward)
    ...
    def _current_quant_norm(self) -> float:
        return float(np.clip(float(self.current_quant_bits) / float(max(self.max_quant_bits, 1)), 0.0, 1.0))

    def _current_sample_packet_bits(self) -> float:
        bit_scale = float(self.current_quant_bits) / 8.0
        return float(self.sensing_band_num) * float(self.config.comm.data_per_sample) * bit_scale
    ...
    def _apply_uav_joint_action(self, action: int) -> int:
        ...
        _, moved_steps = self._move_uav(direction_idx=direction_idx)
        return int(moved_steps)
    ...
    def _move_uav(self, direction_idx: int) -> Tuple[float, int]:
        ...
        return float(energy), int(moved_steps)

    def _move_ugv(self, direction: int) -> int:
    ...
    def _enqueue_sample_packet(self, sample: SpectrumSample, novelty_ratio: float = 1.0) -> float:
        data_size = self._current_sample_packet_bits()
            ...
            if cached_outputs is not None:
                ...
                try:
                    mean_map, var_map, _ = ensemble_reconstruct_maps(
                        ...
                        seed=self.config.ippo.seed + int(seed_offset),
    ...
    def _compute_reward(
        ...
        # Backlog is driven by relay behavior, while drops are driven by UAV sampling volume.
        r_queue_backlog = -rc.gamma_queue * float(queue_bits_norm)
        r_queue_drop = -rc.gamma_queue * float(dropped_norm)
        r_queue = r_queue_backlog + r_queue_drop
        ...
        team_reward = uav_reward + ugv_reward
        ...
        info = {
            ...
            "r_queue_backlog": float(r_queue_backlog),
            "r_queue_drop": float(r_queue_drop),
    ...
    def _extract_uav_quant_unc_features(
        ...
        var_map_age_steps = max(0, int(self.current_step) - int(getattr(self, "latest_var_map_update_step", 0)))
        var_map_age_norm = float(
            np.clip(var_map_age_steps / float(max(1, self.ensemble_refresh_interval)), 0.0, 1.0)
        )
    ...
    def _pending_ensemble_norm(self) -> float:
        return float(
            np.clip(
                float(self.pending_ensemble_effective_sample_count)
                / float(max(1, self.ensemble_refresh_interval)),
                0.0,
                1.0,
            )
        )
    ...
    def _extract_uav_quant_context_features(
        ...
        features = np.array(
            [
                ...
                float(np.asarray(los_link_obs, dtype=float).reshape(-1)[0]),
                self._pending_ensemble_norm(),
            ],
            dtype=float,
        )
        ...
        info.update(
            {
                ...
                "var_map_age_steps": int(
                    max(0, int(self.current_step) - int(getattr(self, "latest_var_map_update_step", 0)))
                ),
...
class VecUAVUGVEnvironment(_VecResetSeedMixin):
    ...
    def __init__(
        ...
        self.base_seed = int(config.ippo.seed)

...
def parse_args():
    ...
    parser.add_argument(
        "--uav_quant_bits",
        type=str,
        default=",".join(str(bits) for bits in defaults.uav.quant_bits),
        help="Comma-separated UAV quantization bit choices, e.g. 4,6,8.",
    )
    parser.add_argument(
        "--uav_default_quant_bits",
        type=int,
        default=defaults.uav.default_quant_bits,
        help="Default UAV quantization bit depth used at reset.",
    )
    ...
    parser.add_argument(
        ...
        help="II-BTD reconstruction backend: du_iibtd_sr, du_iibtd_sr_learn_nu, or gpu.",
    )
    parser.add_argument(
        "--iibtd_device",
        type=str,
    ...
    parser.add_argument(
        "--iibtd_du_checkpoint_path",
        type=str,
        default=defaults.planner.iibtd_du_checkpoint_path,
        help="Comma-separated checkpoint path(s) for DU-IIBTD SR backends.",
    )
    parser.add_argument(
        "--iibtd_du_min_sensors_for_update",
        type=int,
        default=defaults.planner.iibtd_du_min_sensors_for_update,
        help="Minimum fused sensors before DU-IIBTD applies an unrolled update.",
    )
    parser.add_argument(
        "--iibtd_du_update_batch_size",
        type=int,
        default=defaults.planner.iibtd_du_update_batch_size,
        help="Fused-observation batch size for DU-IIBTD updates; <=0 uses one batch.",
    )
...
def build_config(args) -> Config:
    ...
    config.uav.quant_bits = [
        int(item)
        for item in str(args.uav_quant_bits).replace(";", ",").split(",")
        if str(item).strip()
    ]
    config.uav.default_quant_bits = int(args.uav_default_quant_bits)
    ...
    config.planner.iibtd_du_checkpoint_path = (
        str(args.iibtd_du_checkpoint_path).strip()
        or config.planner.iibtd_du_checkpoint_path
    )
    config.planner.iibtd_du_min_sensors_for_update = max(
        1, int(args.iibtd_du_min_sensors_for_update)
    )
    config.planner.iibtd_du_update_batch_size = max(
        0, int(args.iibtd_du_update_batch_size)
    )
...
def _resolve_iibtd_runtime_backend(config: Config) -> str:
    backend_key = str(config.planner.iibtd_backend).strip().lower() or "du_iibtd_sr"
    backend_aliases = {
    ...
    backend = backend_aliases.get(backend_key, backend_key)
    if backend in {"du_iibtd_sr", "du_iibtd_sr_learn_nu"}:
        return backend
    if backend == "gpu":
        if torch is not None and torch.cuda.is_available():
            return "gpu"
        raise RuntimeError("backend=gpu requires CUDA; cpu/auto II-BTD backends are disabled.")
    raise ValueError(
...
def _apply_runtime_backend_policy(config: Config) -> Optional[str]:
    ...
    if iibtd_backend not in {"gpu", "du_iibtd_sr", "du_iibtd_sr_learn_nu"} or cuda_count <= 0:
        return None
...
def train(config: Config, cli_args: Optional[argparse.Namespace] = None):
    ...
    try:
        ...
        print(
            ...
            f"reconstruction backend={_resolve_iibtd_runtime_backend(config)} | "
            f"reconstruct device={_resolve_iibtd_runtime_device(config)}"
        )

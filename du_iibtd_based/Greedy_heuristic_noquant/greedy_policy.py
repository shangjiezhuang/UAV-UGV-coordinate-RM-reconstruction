...
class GreedyPathPolicy:
    ...
    def select_action(self, env, uav_action_mask=None) -> int:
        ...
        action = self._encode_action(env, direction_idx=direction, bw_idx=bw_idx)
        action = self._repair_action_if_masked(env, action, direction, bw_idx, action_mask)
    ...
    def _encode_action(self, env, direction_idx: int, bw_idx: int) -> int:
    ...
    def _repair_action_if_masked(
        ...
        for candidate_bw in self._preferred_bandwidth_order(env, bw_idx):
            candidate = self._encode_action(env, direction_idx, candidate_bw)

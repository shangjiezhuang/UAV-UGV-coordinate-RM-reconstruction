...
- UAV action jointly controls movement direction, sensing-communication bandwidth split, and quantization bit depth.
...
_UAV_QUANT_UNC_FEATURE_NAMES = (
    "current_quant_norm",
    "local_unc_mean",
    "local_unc_max",
    "local_unc_min",
    "current_cell_unc_mean",
    "target_cell_unc_mean",
    "target_freq_unc",
    "var_map_age_norm",
)

_UAV_QUANT_CONTEXT_FEATURE_NAMES = (
    "snr_norm",
)
...
def build_env_instance(
    ...
    seed = int((env_config.mappo.seed if seed_base is None else seed_base) + int(env_idx))
    sim_data = SimDataGen(
        config=env_config,
        seed=seed,
        ...
        while True:
            ...
            try:
                if cmd == "reset":
                    ...
                    step_ugv_action = None if ugv_action is None else int(ugv_action)
                    obs, rew, term, trunc, info = env.step(int(uav_action), step_ugv_action)
                    if term or trunc:
...
def _pack_vec_step_results(
    ...
    rewards = {
        "team_reward": np.array([r[1]["team_reward"] for r in results], dtype=np.float32),
    }
...
class UAVUGVEnvironment:
    ...
    def __init__(
        ...
        self.quant_bits = np.asarray(config.uav.quant_bits, dtype=int)
        if self.quant_bits.size == 0:
            raise ValueError("config.uav.quant_bits must not be empty")
        self.num_quant_choices = int(self.quant_bits.size)
        self.default_quant_bits = int(config.uav.default_quant_bits)
        self.current_quant_bits = int(self.default_quant_bits)
        default_quant_matches = np.flatnonzero(self.quant_bits == self.default_quant_bits)
        self.current_quant_choice_idx = int(default_quant_matches[0]) if default_quant_matches.size else 0
        ...
        self.hybrid_global_hold_intervals = max(
            1, int(config.planner.hybrid_global_hold_intervals)
        )
        ...
        # UAV action: choose movement direction + bandwidth split + quantization bit depth.
        self.uav_action_size = (
            self.uav_direction_choices * self.num_bw_choices * self.num_quant_choices
        )
        ...
        self._init_log_quantization_range()
    ...
    def _build_planner_common_info(
        self,
        *,
        local_candidate_count: Optional[int] = None,
        planner_mode_switch: str = "",
        planner_mode_switch_reason: str = "",
    ) -> Dict[str, object]:
        if local_candidate_count is None:
            local_candidate_count = self._count_planner_candidate_grids(mode_override="local")
        return {
            ...
            "planner_target_mode": str(self.target_mode),
            "planner_submode": str(self.planner_submode),
            "planner_effective_target_mode": str(self._effective_planner_mode()),
    ...
    def _setup_observation_spaces(self) -> None:
        c = self.config
        self.uav_quant_unc_feature_dim = len(_UAV_QUANT_UNC_FEATURE_NAMES)
        self.uav_quant_context_feature_dim = len(_UAV_QUANT_CONTEXT_FEATURE_NAMES)
        # uav: position(2) + energy(1) + queue(1) + bw_ratio(1)
        #    + quant/uncertainty features + quant-choice context
        #    + local_goal(dx, dy, dist, center_freq)
        #    + ugv_position(2)
        self.uav_obs_dim = (
            2 + 1 + 1 + 1
            + self.uav_quant_unc_feature_dim
            + self.uav_quant_context_feature_dim
            + 4 + 2
        )
        # state: uav_position(2) + ugv_position(2) + uav_energy(1) + snr(1)
        #    + queue(1) + nmse(1) + bw_ratio(1) + los_link(1)
        #    + quant/uncertainty features
        #    + planner_state(5)
        self.critic_state_dim = (
            4 + 1 + 1 + 1 + 1 + 1 + 1
            + self.uav_quant_unc_feature_dim
            + c.obs.num_planner_features
        )
    ...
    def get_action_dims(self) -> Dict[str, int]:
        return {
            ...
            "uav_quant_bits": self.num_quant_choices,
        }
    ...
    def _init_cached_constants(self) -> None:
        ...
        self.max_quant_bits = int(np.max(self.quant_bits))
        self.quant_log_offset = 1e-6
    ...
    def _init_log_quantization_range(self) -> None:
        values = np.asarray(self.ground_truth_map, dtype=float)
        log_values = np.log(np.maximum(values, 0.0) + self.quant_log_offset)
        finite = log_values[np.isfinite(log_values)]
        if finite.size == 0:
            self.quant_log_min = 0.0
            self.quant_log_max = 1.0
            return
        self.quant_log_min = float(np.min(finite))
        self.quant_log_max = float(np.max(finite))
        if self.quant_log_max <= self.quant_log_min + 1e-12:
            self.quant_log_max = self.quant_log_min + 1.0
    ...
    def reset(self, seed: Optional[int] = None) -> Tuple[Dict[str, np.ndarray], dict]:
        ...
        self._set_quantization_info(self.default_quant_bits)
        ...
        self.latest_var_map_update_step = 0
        ...
        info = {
            ...
            **self._build_planner_common_info(),
            ...
            **self._build_quant_uncertainty_info(current_target=current_target),
        }
        return obs, info
    ...
    def _snapshot_global_switch_top_targets(self, top_k: int = 3) -> None:
        if self._effective_planner_mode() != "global":
        ...
        global_candidate_mask = self._build_local_candidate_mask(mode_override="global")
        target_dicts = select_top_k_grid_candidates(
            ...
            candidate_mask=global_candidate_mask,
        )
        self.global_switch_top_targets = [
            PlannerTarget(
                gx=int(target["gx"]),
    ...
    def _maybe_update_hybrid_planner_mode(self, map_updated: bool) -> Tuple[str, str, int]:
        ...
        if self.planner_submode == "local":
            ...
            self.global_steps_remaining = (
                int(self.hybrid_global_hold_intervals) * int(self.ensemble_refresh_interval)
            )
    ...
    def _sync_cached_ensemble_state(self) -> None:
        ...
        self.latest_var_map_update_step = int(self.current_step)
        self.uncertainty = UncertaintyMap(
    ...
    def _run_ensemble_map_update(
        ...
        return self._record_ensemble_event(
            ...
            extra_fields={
                ...
                "member_kernel_bandwidths": ensemble_diag.get("member_kernel_bandwidths", []),
                "member_du_checkpoint_paths": ensemble_diag.get("member_du_checkpoint_paths", []),
                "member_last_incremental_accept_counts": ensemble_diag.get("member_last_incremental_accept_counts", []),
                "member_incremental_updated_mask": ensemble_diag.get("member_incremental_updated_mask", []),
            },
        )
    ...
    def step(
        ...
        if sample is not None:
            data_produced_bits = self._enqueue_sample_packet(
                sample,
        ...
        info = {
            ...
            **self._build_quant_uncertainty_info(current_target=info_current_target),
            ...
            **self._build_planner_common_info(
                ...
                planner_mode_switch_reason=planner_mode_switch_reason,
            ),
            "target_reached": int(target_reached),
            ...
            **self._build_global_switch_top_target_info(top_k=3),
            ...
            "ensemble_member_kernel_bandwidths": (
                ensemble_event.get("member_kernel_bandwidths", [])
                if ensemble_event is not None
                else []
            ),
            "ensemble_member_du_checkpoint_paths": (
                ensemble_event.get("member_du_checkpoint_paths", [])
                if ensemble_event is not None
                else []
            ),
            "ensemble_member_last_incremental_accept_counts": (
                ensemble_event.get("member_last_incremental_accept_counts", [])
                if ensemble_event is not None
                else []
            ),
            "ensemble_member_incremental_updated_mask": (
                ensemble_event.get("member_incremental_updated_mask", [])
                if ensemble_event is not None
                else []
            ),
    ...
    def _set_quantization_info(self, bits: int) -> None:
        bits = int(bits)
        matches = np.flatnonzero(self.quant_bits == bits)
        if matches.size:
            bit_choice_idx = int(matches[0])
        else:
            bit_choice_idx = int(np.argmin(np.abs(self.quant_bits.astype(float) - float(bits))))
            bits = int(self.quant_bits[bit_choice_idx])
        self.current_quant_choice_idx = int(bit_choice_idx)
        self.current_quant_bits = int(bits)

    def _current_quant_norm(self) -> float:
        return float(
            np.clip(
                float(self.current_quant_bits) / float(max(self.max_quant_bits, 1)),
                0.0,
                1.0,
            )
        )

    def _current_sample_packet_bits(self, sample: SpectrumSample) -> float:
        observed_band_count = int(np.asarray(sample.freq_band_indices).size)
        observed_band_count = int(np.clip(observed_band_count, 0, self.K))
        bit_scale = float(self.current_quant_bits) / 8.0
        return (
    ...
    def _quantize_log_dequantize_measurements(self, values: np.ndarray, bits: int) -> np.ndarray:
        arr = np.asarray(values, dtype=float)
        bits = max(1, int(bits))
        levels = float((1 << bits) - 1)
        log_values = np.log(np.maximum(arr, 0.0) + self.quant_log_offset)
        span = max(float(self.quant_log_max - self.quant_log_min), 1e-12)
        norm = np.clip((log_values - self.quant_log_min) / span, 0.0, 1.0)
        q = np.round(norm * levels)
        log_hat = (q / levels) * span + self.quant_log_min
        gamma_hat = np.exp(log_hat) - self.quant_log_offset
        return np.maximum(gamma_hat, 0.0).astype(float)

    def _decode_uav_action(self, action: int) -> Tuple[int, int, int]:
        action = int(np.clip(action, 0, self.uav_action_size - 1))
        bit_choice_idx = action % self.num_quant_choices
        action_without_bit = action // self.num_quant_choices
        bw_choice_idx = action_without_bit % self.num_bw_choices
        direction_choice_idx = action_without_bit // self.num_bw_choices
        direction_idx = int(self.uav_direction_ids[direction_choice_idx])
        return direction_idx, bw_choice_idx, bit_choice_idx

    def _apply_uav_joint_action(self, action: int) -> None:
        direction_idx, bw_choice_idx, bit_choice_idx = self._decode_uav_action(action)
        self._set_bandwidth_info(float(self.bandwidth_ratios[bw_choice_idx]))
        self._set_quantization_info(int(self.quant_bits[bit_choice_idx]))
    ...
    def _enqueue_sample_packet(self, sample: SpectrumSample, novelty_ratio: float = 1.0) -> float:
        data_size = self._current_sample_packet_bits(sample)
    ...
    def _collect_grid_sample(
        ...
        gamma_full = self._quantize_log_dequantize_measurements(
            gamma_full,
            bits=int(self.current_quant_bits),
        )
            ...
            if cached_outputs is not None:
                ...
                try:
                    mean_map, var_map, _ = ensemble_reconstruct_maps(
                        ...
                        du_checkpoint_path=self.config.planner.iibtd_du_checkpoint_path,
                        du_min_sensors_for_update=self.config.planner.iibtd_du_min_sensors_for_update,
                        du_update_batch_size=self.config.planner.iibtd_du_update_batch_size,
                    )
                except Exception as exc:
                    raise RuntimeError(
    ...
    def _uncertainty_norm_den(self) -> float:
        if not hasattr(self, "latest_var_map"):
            return 1.0
        max_unc = float(np.nanmax(np.asarray(self.latest_var_map, dtype=float)))
        if not np.isfinite(max_unc) or max_unc <= 1e-12:
            return 1.0
        return max_unc

    def _normalize_uncertainty_value(self, value: float, den: Optional[float] = None) -> float:
        if den is None:
            den = self._uncertainty_norm_den()
        return float(np.clip(float(value) / float(max(den, 1e-12)), 0.0, 1.0))

    def _extract_uav_quant_unc_features(
        self,
        ...
        var_map = np.asarray(self.latest_var_map, dtype=float)
        spatial_unc = np.mean(var_map, axis=2)
        unc_den = self._uncertainty_norm_den()

        ux, uy = self._grid_cell(self.uav_pos)
        radius = max(1, int(self.local_planner_radius))
        x0 = max(0, ux - radius)
        x1 = min(self.Nx, ux + radius + 1)
        y0 = max(0, uy - radius)
        y1 = min(self.Ny, uy + radius + 1)

        local_unc = spatial_unc[x0:x1, y0:y1]
        local_dx = np.abs(self.grid_x_coords[x0:x1, y0:y1] - float(ux))
        local_dy = np.abs(self.grid_y_coords[x0:x1, y0:y1] - float(uy))
        local_mask = (local_dx + local_dy) <= float(radius)
        local_mask &= self.sampling_valid_mask[x0:x1, y0:y1]
        if np.any(local_mask):
            local_values = local_unc[local_mask]
        else:
            local_values = np.asarray([spatial_unc[ux, uy]], dtype=float)

        current_cell_unc = float(spatial_unc[ux, uy])
        target_cell_unc = 0.0
        target_freq_unc = 0.0
        if current_target is not None:
            tx = int(np.clip(current_target.gx, 0, self.Nx - 1))
            ty = int(np.clip(current_target.gy, 0, self.Ny - 1))
            tf = int(np.clip(current_target.center_freq, 0, self.K - 1))
            target_cell_unc = float(spatial_unc[tx, ty])
            target_freq_unc = float(var_map[tx, ty, tf])

        var_map_age_steps = max(
            0,
            int(self.current_step) - int(getattr(self, "latest_var_map_update_step", 0)),
        )
        var_map_age_norm = float(
            np.clip(
                var_map_age_steps / float(max(1, self.ensemble_refresh_interval)),
                0.0,
                1.0,
            )
        )
        ...
        features = np.array(
            [
                self._current_quant_norm(),
                self._normalize_uncertainty_value(float(np.mean(local_values)), unc_den),
                self._normalize_uncertainty_value(float(np.max(local_values)), unc_den),
                self._normalize_uncertainty_value(float(np.min(local_values)), unc_den),
                self._normalize_uncertainty_value(current_cell_unc, unc_den),
                self._normalize_uncertainty_value(target_cell_unc, unc_den),
                self._normalize_uncertainty_value(target_freq_unc, unc_den),
                var_map_age_norm,
            ],
            dtype=float,
        )
    ...
    def _pending_ensemble_norm(self) -> float:
        pending_effective = int(
            getattr(
                self,
                "pending_ensemble_effective_sample_count",
                int(getattr(self, "pending_ensemble_sample_count", 0)),
            )
        )
        return float(
            np.clip(
                float(pending_effective) / float(max(1, self.ensemble_refresh_interval)),
                0.0,
                1.0,
            )
        )

    def _extract_uav_quant_context_features(
        self,
        ...
        features = np.array(
            [
                float(np.clip(snr_norm, -1.0, 1.0)),
            ],
            dtype=float,
        )
        return np.nan_to_num(features, nan=0.0, posinf=1.0, neginf=-1.0)

    def _build_quant_uncertainty_info(
        self,
        current_target: Optional[PlannerTarget] = None,
    ) -> Dict[str, float]:
        features = self._extract_uav_quant_unc_features(current_target=current_target)
        info = {
            name: float(features[idx])
            for idx, name in enumerate(_UAV_QUANT_UNC_FEATURE_NAMES)
        }
        info.update(
            {
                "quant_bits": int(self.current_quant_bits),
                "quant_choice_idx": int(self.current_quant_choice_idx),
                "num_quant_choices": int(self.num_quant_choices),
                "var_map_age_steps": int(
                    max(
                        0,
                        int(self.current_step) - int(getattr(self, "latest_var_map_update_step", 0)),
                    )
                ),
                "pending_ensemble_norm": self._pending_ensemble_norm(),
                "quant_log_min": float(self.quant_log_min),
                "quant_log_max": float(self.quant_log_max),
            }
        )
        return info
    ...
    def _build_uav_obs(
        ...
        parts.append(self._extract_uav_quant_unc_features(current_target=current_target))
        parts.append(
            self._extract_uav_quant_context_features(
                snr_norm=snr_norm,
            )
        )
    ...
    def _build_uav_action_mask(self) -> np.ndarray:
        ...
        mask = np.repeat(valid_dirs, self.num_bw_choices * self.num_quant_choices)
    ...
    def _encode_local_goal_for_uav_obs(
        ...
        if current_target is not None:
            ...
            goal_direction = self._normalize_target_delta(
                self.uav_pos,
                goal_pos,
                distance=goal_dist,
            )
...
class VecUAVUGVEnvironment(_VecResetSeedMixin):
    ...
    def step(
        ...
        ugv_actions: Optional[np.ndarray] = None,
    ) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray], np.ndarray, np.ndarray, List[dict]]:
        results = []
        for i, env in enumerate(self.envs):
            step_ugv_action = None if ugv_actions is None else int(ugv_actions[i])
            obs, rew, term, trunc, info = env.step(int(uav_actions[i]), step_ugv_action)
            if term or trunc:
                info["terminal_obs"] = obs
                obs, _ = env.reset(seed=self._next_reset_seed(i))
        ...
        for env_idx in range(self.num_envs):
            self._send_command(
                ...
                (
                    ...
                    None if ugv_actions is None else int(ugv_actions[env_idx]),

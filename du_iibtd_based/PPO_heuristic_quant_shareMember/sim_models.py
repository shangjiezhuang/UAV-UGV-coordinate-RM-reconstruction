...
class IIBTD_opt:
    ...
    def __init__(
        ...
        self._ensemble_member_kernel_bandwidths = np.empty((0,), dtype=float)
    ...
    def _run_full_ensemble_reconstruct(
        self,
        *,
        obs_locs: np.ndarray,
        gamma: np.ndarray,
        omega: np.ndarray,
        fused_effective_count: int,
        seed: int,
        solver_device: str,
    ...
    def reconstruct(self) -> RadioMapState:
        if self._pending_samples:
            ...
            if not self._ensemble_member_models:
                mean_map, var_map, _, ensemble_info = self._run_full_ensemble_reconstruct(
                    obs_locs=obs_locs,
                    gamma=gamma,
                    omega=omega,
                    fused_effective_count=fused_effective_count,
                ...
                mean_map, var_map, _, ensemble_info = incremental_refresh_ensemble_models(
                    ...
                    nu=self.config.planner.iibtd_nu,
                    kernel_bandwidth=self.config.planner.iibtd_kernel_bandwidth,
                    member_kernel_bandwidths=self._ensemble_member_kernel_bandwidths,
                    solver_backend=self.config.planner.iibtd_backend,
                    solver_device=solver_device,
                    gpu_phi_solver=self.config.planner.iibtd_gpu_phi_solver,
                    du_checkpoint_path=self.config.planner.iibtd_du_checkpoint_path,
            ...
            next_member_kernel_bandwidths = np.asarray(
                ensemble_info.get(
                    "member_kernel_bandwidths",
                    self._ensemble_member_kernel_bandwidths,
                ),
                dtype=float,
            ).copy()
            ...
            self._ensemble_member_kernel_bandwidths = next_member_kernel_bandwidths
...
class GridScene(_BuildingHeightsMixin):
